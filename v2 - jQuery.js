...
const books = [
  ...
  const borrowBook = (memberName, bookTitle) => {
    const member = $.grep(members, ({ name }) => name === memberName)[0];
    const book = $.grep(books, ({ title }) => title === bookTitle)[0];
    ...
    if (member.borrowed_books.length > 0)
      return console.log(
        `${memberName} already has a borrowed book. Return it first.`
      );
    if (book.isBorrowed)
      return console.log(
        `Book '${bookTitle}' is already borrowed. We will notify you when it returns.`
      );
  ...
  const returnBook = (memberName, bookTitle) => {
    ...
    const bookIndex = $.inArray(book.title, member.borrowed_books);
    if (bookIndex === -1)
  ...
  const showStatus = (members, books) => {
    ...
    $.each(
      $.grep(books, ({ isBorrowed }) => !isBorrowed),
      (index, { title }) => {
        console.log(`- ${title}`);
      }
    );
    ...
    $.each(members, (index, { name, borrowed_books }) => {
      console.log(
